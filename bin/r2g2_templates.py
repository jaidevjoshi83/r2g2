...
def generate_rscript_content(input_names, rname, r_name):
    """Generate R script content for the Galaxy tool.
    
    Args:
        input_names: List of processed input names
        rname: Name of the R function
        r_name: Name of the R library
        
    Returns:
        String containing the R script content
    """
    rscript_content = f'{CONFIG_SPLIT_DESIRED_OUTPUTS}\nlibrary({r_name})\n#set $___USE_COMMA___ = ""\nrval <- {rname}('
    
    for inp_name, input_placeholder, input_type, use_quotes in input_names:
        if input_type == 'ellipsis':
            rscript_content += '''${___USE_COMMA___}
                            #set $___USE_COMMA___ = ","
            ...
            if input_type == 'dataset':
                rscript_content += f'${{___USE_COMMA___}}\n#set $___USE_COMMA___ = ","\n{inp_name} = readRDS("${{{input_placeholder}_type.{input_placeholder}}}")'
...
def create_tool_xml(package_name, rname, r_name, galaxy_tool_version, package_obj):
    """Create a Galaxy tool XML definition for an R function.
    ...
    Returns:
        Dictionary containing the XML definition
    """
    xml_dict = {
        'package_name': package_name,
    ...
    # Simplify ID for ToolShed compatibility
    xml_dict['id_underscore'] = simplify_text(xml_dict['id'])
    xml_dict['id'] = simplify_text(xml_dict['id'])
    
    # Process help documentation
    help_pages = get_r_help_pages(rname)
    try:
        join_char = ""
    ...
    inputs, input_names, xml_dict['inputs'] = process_function_inputs(package_obj)
    
    # Generate R script content
    xml_dict['rscript_content'] = generate_rscript_content(input_names, rname, r_name)
    
    # Generate the complete tool XML
    xml_dict['tool_xml'] = tool_xml % xml_dict
